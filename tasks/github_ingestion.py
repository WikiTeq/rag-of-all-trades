...
class GitHubIngestionJob(IngestionJob):
    ...
    def __init__(self, config: dict):
        ...
        if branch and commit_sha:
            raise ValueError(
                "branch and commit_sha are mutually exclusive in GitHub connector config"
            )
        # Default to "main" if neither is provided
        self.branch: Optional[str] = branch or (None if commit_sha else "main")
        self.commit_sha: Optional[str] = commit_sha or None
        ...
        if has_app:
            ...
            github_client = GithubClient(github_app_auth=app_auth)
            issues_client = GitHubIssuesClient(github_app_auth=app_auth)
        else:
            github_client = GithubClient(github_token=personal_token)
    ...
    def list_items(self) -> Iterator[IngestionItem]:
        ...
        if self.include_issues:
            ...
            try:
                ...
                logger.error(
                    f"[{self.source_name}] Failed to load issues: {e}"
                )
                issues = []
    ...
    def _build_label_filters(self):
        ...
        if self._exclude_labels:
            return [
                (label, GitHubRepositoryIssuesReader.FilterType.EXCLUDE)
