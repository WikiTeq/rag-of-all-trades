...
class TestMediaWikiIngestionJob(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = {
            "name": "test_wiki",
            "config": {"api_url": "https://example.com/w/api.php"},
        }
    ...
    def test_get_page_info_success(self, mock_session_class):
        ...
        mock_url_response = self._create_mock_response(
            json_data={
                "query": {
                    "pages": {
                        "123": {
        ...
        mock_parse_response = self._create_mock_response(
            json_data={
                "parse": {"text": {"*": "<p>Test page content with <a href='/wiki/Links'>links</a> and templates.</p>"}}
            }
        )
    ...
    def test_get_page_info_missing_page(self, mock_session_class):
        ...
        mock_response = self._create_mock_response(
            json_data={"query": {"pages": {"-1": {"title": "Missing Page", "missing": True}}}}
        )
    ...
    def test_get_page_url_success(self, mock_session_class):
        ...
        mock_parse_response = self._create_mock_response(json_data={"parse": {"text": {"*": "<p>Content</p>"}}})
    ...
    def test_initialization_validation(self, mock_session_class):
        ...
        with self.assertRaises(ValueError):
            # Missing api_url
            MediaWikiIngestionJob({"name": "test", "config": {}})
    ...
    def test_list_items_success(self, mock_sleep, mock_session_class):
        ...
        mock_allpages_response = self._create_mock_response(
            json_data={
                "query": {
                    "allpages": [
                        {"title": "Test Page 1"},
        ...
        mock_timestamp_response = self._create_mock_response(
            json_data={
                "query": {
                    "pages": {
                        "123": {
    ...
    def test_get_raw_content_success(self, mock_session_class):
        ...
        # Mock parse response (second API call) - returns HTML
        mock_parse_response = self._create_mock_response(json_data={"parse": {"text": {"*": "<p>Test content</p>"}}})
        ...
        self.assertEqual(
            item._metadata_cache.get("page_url"),
            "https://example.com/wiki/Test_Page",
        )
    ...
    def test_get_all_pages_single_page(self, mock_session_class):
        ...
        mock_response = self._create_mock_response(
            json_data={"query": {"allpages": [{"title": "Page 1"}, {"title": "Page 2"}]}}
        )
        mock_session.get.return_value = mock_response
    ...
    def test_get_all_pages_pagination(self, mock_sleep, mock_session_class):
        ...
        second_response = self._create_mock_response(
            json_data={"query": {"allpages": [{"title": "Page 3"}, {"title": "Page 4"}]}}
        )
    ...
    def test_process_item_success(self, mock_session_class, mock_sleep):
        ...
        mock_parsed_response = self._create_mock_response(
            json_data={"parse": {"text": {"*": "<p>This is parsed content.</p>"}}}
        )
    ...
    def test_process_item_duplicate_content(self, mock_session_class, mock_sleep):
        ...
        # Mock parse response (second API call)
        mock_parsed_response = self._create_mock_response(
            json_data={"parse": {"text": {"*": "<p>Duplicate content.</p>"}}}
        )
